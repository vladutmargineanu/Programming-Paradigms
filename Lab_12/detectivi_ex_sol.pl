...
% Predicatul findall pune în Bag câte un element
% Template pentru fiecare soluție a expresiei Goal. 
au_pusca(_ListaNume) :- findall(Om, inarmat(Om, pusca), _ListaNume).
...
% Predicatul bagof seamănă cu findall, diferența fiind că bagof construiește câte 
% o listă Bag pentru fiecare instanțiere diferită a variabilelor libere din Goal.
exercitiul(3, [1, puncte]).

au_arma(_Arma, _ListaNume) :- bagof(Om, Om^inarmat(Om, _Arma), _ListaNume).

au_marca(_Marca, _ListaNume) :- bagof(Om, Om^conduce(Om, _Marca), _ListaNume).
...
% Predicatul findall pune în Bag câte un element
% Template pentru fiecare soluție a expresiei Goal. 
arme_bmw(_ListaArme) :- findall(Arma, (conduce(Om, bmw), inarmat(Om, Arma)), _ListaArme).
...
% Predicatul bagof seamănă cu findall, diferența fiind că bagof construiește câte 
% o listă Bag pentru fiecare instanțiere diferită a variabilelor libere din Goal.
arme_marca(_Marca, _ListaArme) :- bagof(Arma, Om^(conduce(Om, _Marca), inarmat(Om, Arma)), _ListaArme).
...
% Predicatul findall pune în Bag câte un element
% Template pentru fiecare soluție a expresiei Goal. 
marci_arma_unica(_ListaMarci) :- findall(Marca, arme_marca(Marca, [_]), _ListaMarci).
...
suspect1(Nume:Marca:Arma) :- suspect(Nume:Marca:Arma),
        ...
        findall(Arma1, (member(Arma1, ListaArme), au_arma(Arma1,[_])), []).
...
suspect2(Nume:Marca:Arma) :- suspect1(Nume:Marca:Arma),
           findall(Marca, suspect1(_:Marca:_),[_,_|_]). % TODO
...
suspect3(Nume:Marca:Arma) :- suspect2(Nume:Marca:Arma),
        findall(Om, suspect2(Om:_:Arma),[_,_|_]). % TODO
...
suspect4(Nume:Marca:Arma) :- suspect3(Nume:Marca:Arma),
        findall(Marca, suspect3(_:Marca:_),[_,_|_]). %TODO
