...
{-  data = permite definirea de noi tipuri de date algebrice 
 numele constructorilor (denumiți și constructori de date) cu pipeline -}
data SList a = UndefinedSList | Elem a | List [SList a] deriving Show
{- Elem a = Check that a value is an element of a list
   List = constructor pentru a realiza un element de tip list in lista mare
-}
emptySList :: SList a
emptySList = List []
{- consElem, adaugă un element în capul unei liste -}
...
{- daca lista are mai mult de doua elemente,  -}
...
deepEqual (List xs) (List ys)
  {- zipWith: generalizare a lui zip care, aplică o funcție binară pe elementele a două liste -}
  | length xs == length ys = and $ zipWith deepEqual xs ys
  | otherwise              = False
{- flatten, întoarce lista cu elementele din SList (pe același nivel) -}
flatten :: SList a -> [a]
flatten (Elem x) = [x]
{- concatMap = aplica o funcție peste toate elementele unui container și concateneaza
listele rezultate. -}
...
findElem (BSTNode crtNode leftNode rightNode) x
  ...
  | otherwise    = findElem rightNode xs
