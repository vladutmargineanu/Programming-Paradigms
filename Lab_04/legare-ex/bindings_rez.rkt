...
(define (compute-length get-line-segment get-start-point get-end-point)
  ;; luam segmentul de dreapta in segment
  (let* ([segment (get-line-segment)]
         ;; start == punctul din care incepe segmentul
         [start (get-start-point segment)]
         ;; end == punctul in care se termina segmentul
         ;; start si end reprezinta fiecare o perehe de coordonate
  ...
  (define (distance p1 p2)
    ...
    (let* ([segment (cons p1 p2)]
           [get-segment (λ() segment)]
           ;; get-segment argument pt fctia get-line-segment => întoarce un segment de dreaptă.
           [get-start (λ(segment) p1)]
           ;; get-start argument pt fctia get-start-point => primește un segment de dreaptă și
           ;; întoarce punctul din care segmentul începe.
           [get-end (λ(segment) p2)])
           ;; get-end argument pt fctia  get-end-point => primește un segment de dreaptă și
           ;;întoarce punctul în care segmentul se termină.
      (compute-length get-segment get-start get-end)))

  ;;Evaluarea oricărei λ-expresii produce o închidere funcțională.
  ;;închiderea funcționala este că pot fi folosite pentru a întârzia evaluarea.
  ;;textul nu este evaluat înainte de aplicarea λ-expresiei pe argumente
...
(check-exp-part 'a 1 (distance '(9 . 12)  '(12 . 16)) 5)
  ...
  (define (compute-f-with-step f a b step)
    ;;mod de a itera în interiorul unei funcții => named let il numim calc-f
    (let calc-f ([x a])
      ;;aici e vizibil x-ul parametru pentru iter
      (if (> x b)
          '()
          ;; calc-f este apelată recursiv ca o funcție de 1 parametru
          ;; cu cons formam rezultatul final lista cu valorile functiei (f x)
          (cons (f x) (calc-f (+ x step))))))
  ...
  (define (num-concat x y)
    (let* ( ;; domeniul de definitie din let*
           ;; definim functia '+' cu λ
           [+ (λ(x y) (string->number (string-append x y)))]
           ;; definim x ca fiind rezultatul returnat de fctia (numer->string x)
          [x (number->string x)]
           ;; definim y ca fiind rezultatul returnat de fctia (numer->string y)
          [y (number->string y)])
      ;; corpul functiei care realizeaza concatenarea x+y
  ...
  (define (compute-perimeter points)
    ;;mod de a itera în interiorul unei funcții => named let il numim perimeter
    (let perimeter ([p points])  ;; demeniul de definitie
      ;; corpul functiei
      ;; daca lista e formata doar din doua elemente intoarcem distanta dintre cele 2 puncte
      (if (null? (cdr p))  ;; daca e null asta == cdr (intoarcem restul listei p (fara primul) in p) facem distance 
          (distance (car points) (car p))  
          ;; altfel facem distanta dintre primele 2 puncte consecutive
          ;; si apelam perimeter de lista p fara primul element (cdr p)
          (+ (distance (car p) (cadr p)) (perimeter (cdr p))))))  ;; cadr ia al doilea element din lista p
          ;; apelam recursiv perimeter pe restul listei p si facem suma distantelor totale
  ...
  (define (3-sequence-max numbers separator)
    ;;let*-values == Like let*, except that each val-expr must produce as many values as corresponding ids
    ;; split-at == Returns the list after the first pos elements of lst ;;(split-at lst pos)
    ;; functia λ returneaza separatorul
    (let*-values ([(secv1 rest1) (splitf-at numbers (λ(x) (not (= x separator))))]
                  [(secv2 rest2) (splitf-at (cdr rest1) (λ(x) (not (= x separator))))]
                  [(secv3 rest3) (splitf-at (cdr rest2) (λ(x) (not (= x separator))))])
      (max (sum secv1) (sum secv2) (sum secv3))))
