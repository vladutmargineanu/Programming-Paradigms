...
% dată.
parseState([Mal, ME, CE, NV, CV], Mal, ME, CE, NV, CV).
...
% S1.
next_state(misionari, State, Next) :-
    parseState(State, Mal, ME, CE, MV, CV),
    boat(M, C),
    ((Mal = est,
    M =< ME,
    C =< CE,
    ...
    NMV is MV + M,
    NCV is CV + C,
    safeMisionari(NME, NCE),
    safeMisionari(NMV, NCV),
    Next = [vest, NME, NCE, NMV, NCV]);
    (Mal = vest,
    M =< MV,
    C =< CV,
    NME is ME + M,
...
preorder([], _).
preorder(Root, Visited) :- pre(Root, [], P), Visited = [Root|P].

pre(Root, [], P) :- arc(Root, Children), pre(Children, [Root], [Root|P]).
pre([], Visited, P) :- reverse(Visited, P).
pre([H|T], Visited, P) :- \+arc(H, _), pre(T, [H|Visited], P).
pre([H|T], Visited, P) :-
    arc(H, Children),
    append(Children, T, ToVisit),
    pre(ToVisit, [H|Visited], P).
