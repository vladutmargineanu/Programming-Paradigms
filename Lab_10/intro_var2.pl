...
myConcat([], L, L).
myConcat([Head|Tail], L, [Head|Rest]) :- myConcat(Tail, L, Rest).
...
myReverse([], []).
myReverse([Head|Tail], List) :- myReverse(Tail, List1), myConcat(List1, [Head], List).
...
myReverseAcc([Head|Tail], Acc, Rev) :- !, myReverseAcc(Tail, [Head|Acc], Rev).
...
extract([Head|Tail], 0, E, [Head|Rest]) :- E1 is E - 1, extract(Tail, 0, E1, Rest).
extract([_|Tail], S, E, Rest) :- S1 is S - 1, E1 is E - 1, extract(Tail, S1, E1, Rest).
...
factorial2(0, 1).
factorial2(N, F) :- factorial2(N1, F1), N is N1 + 1, F is N * F1.
...
sublista([], []).
sublista([Head|Tail], [Head|Res]) :- sublista(Tail, Res).
sublista([_|Tail], Res) :- sublista(Tail, Res).
...
isLeaf(Node) :- nod(Node), not(arc(Node, _)).
...
isRoot(Root) :- nod(Root), not(arc(_, Root)).
...
descendantOf(X, Y) :- nod(X), nod(Y), (arc(Y, X); (arc(P, X), descendantOf(P, Y))).
...
descendants(X, N) :- arc(X, Y), \+ (arc(X, Z), Y \= Z), descendants(Y, N1), N is N1 + 1.
descendants(X, N) :- arc(X, Y), arc(X, Z), Y \= Z, descendants(Y, N1), descendants(Z, N2), N is N1 + N2 + 2.
